      ...
      {rows.map((row) => (
        <li key={row.incident.id}>
          {row.href ? (
            <Link
              href={row.href}
              className="block px-3 py-2 transition-colors hover:bg-background"
            >
    ...
    <div className="flex items-start gap-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          <MonoNumber className="text-[11px] font-semibold text-text-primary">
        ...
        <div className="mt-0.5 truncate text-[10px] text-text-muted">{engineModel}</div>
