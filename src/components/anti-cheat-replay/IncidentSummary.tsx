          ...
          <li key={i.id}>
            <button
              ...
              className={`flex w-full items-start gap-2 px-2 py-2 text-left transition-colors ${
                active ? 'bg-status-warn-dim' : 'hover:bg-background'
              }`}
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <MonoNumber className="text-[11px] text-text-secondary">
                    {formatLapTime(i.tMs)}
                  </MonoNumber>
                </div>
                ...
                <div className="mt-1 text-[10px] text-text-muted">{i.id}</div>
              </div>
            </button>
          </li>
        )
...
function Evidence({ label, value, unit }: { label: string; value: number; unit?: string }) {
  return (
    <span className="flex items-baseline gap-1">
      <span>{label}</span>
      <MonoNumber className="text-text-secondary">{value}</MonoNumber>
      {unit && <span>{unit}</span>}
    </span>
  )
}
